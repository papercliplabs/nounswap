...
import { computePercentDifference } from "@/utils/postProcessing";
import { useCurrencySelector } from "@/components/selectors/CurrencySelector";
import { DATA_FOR_TIME_SELECTOR, useTimeSelector } from "@/components/selectors/TimeSelector";
...
export default function Stats({ data }: StatsProps) {
  ...
  const timeCutoff = timeSelectorData.rangeS
    ? Math.max(Math.floor(Date.now() / 1000) - timeSelectorData.rangeS, minTimestamp)
    : minTimestamp;
  ...
  const treasuryBalance = data
    .map((d) => ({
      ...
      value: currencySelector == "USD" ? d.treasuryBalanceInUsd : d.treasuryBalanceInEth,
    }))
  ...
  const revenueVsCostData = data
    .map((d) => ({
      timestamp: d.timestamp,
      revenue: currencySelector == "USD" ? d.auctionRevenueInUsd : d.auctionRevenueInEth,
  ...
  for (let i = 1; i < revenueVsCostData.length; i++) {
    cumulativeRevenueVsCostData.push({
      ...
      revenue: cumulativeRevenueVsCostData[i - 1].revenue + revenueVsCostData[i].revenue,
      cost: cumulativeRevenueVsCostData[i - 1].cost + revenueVsCostData[i].cost,
    });
  }

  const cumulativeCost = cumulativeRevenueVsCostData[cumulativeRevenueVsCostData.length - 1].cost;
  const cumulativeRevenue = cumulativeRevenueVsCostData[cumulativeRevenueVsCostData.length - 1].revenue;
  ...
  const runway =
    avgDailyProfit > 0 ? Infinity : Math.ceil(treasuryBalance[treasuryBalance.length - 1].value / -avgDailyProfit);

  const descriptionSuffix = `${timeSelectorData.rangeS != undefined ? "the" : ""} ${timeSelectorData.name.toLowerCase()}`;

  const balanceChange = treasuryBalance[treasuryBalance.length - 1].value - treasuryBalance[0].value;
    ...
    <>
      <div className="flex flex-col gap-4 md:flex-row">
        ...
        <StatCard
          ...
          value={formatNumber({ input: cumulativeCost, unit: currencySelector, maxFractionDigits: 0 })}
          className="flex-1"
        />
        <StatCard
          title={`Days left of runway (${timeSelectorData.shortName})`}
          popoverDescription={`An estimate of the number of days the DAO can continue operating based on the revenue and spending over ${descriptionSuffix}. This will be finite when spending exceeds revenue.`}
