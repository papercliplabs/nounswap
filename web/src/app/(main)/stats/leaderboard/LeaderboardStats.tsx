...
export default function LeaderboardStats({ accountLeaderboardData, totalNounsCount }: LeaderboardStatsProps) {
  const uniqueOwnerCount = accountLeaderboardData.length;
  return (
    <>
      <div className="flex flex-col gap-4 md:flex-row">
        <Card className="flex flex-col">
          <TitlePopover title="Total Nouns">Total number of Nouns currently in existence</TitlePopover>
          <span className="label-lg">{formatNumber({ input: totalNounsCount, maxFractionDigits: 0 })}</span>
        </Card>
        <Card className="flex flex-col">
          <TitlePopover title="Unique Owners">
            Total number of unique owners. An owner is considered to be an account with an ownership above 0.1 Nouns.
          </TitlePopover>
      ...
      <Table className="w-full table-fixed overflow-hidden">
        <TableHeader>
          <TableRow className="hover:bg-white">
            ...
            <TableHead>
              Owners <span className="text-content-secondary">({uniqueOwnerCount})</span>
            </TableHead>
        ...
        <TableBody>
          {accountLeaderboardData.map((entry, i) => {
            const ownershipCount = Number(formatUnits(BigInt(entry.effectiveNounsBalance), 24));
            const percentOwnership = ownershipCount / totalNounsCount;
            const address = getAddress(entry.id);
            return (
              <TableLinkExternalRow
                key={entry.id}
                href={CHAIN_CONFIG.chain.blockExplorers?.default.url + `/address/${entry.id}`}
              >
