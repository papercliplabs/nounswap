  ...
  const filteredNouns = useMemo(() => {
    return nouns.filter((noun) => {
      const backgroundMatch =
        filters.background.length === 0 || filters.background.includes(noun.traits.background.seed.toString());
      const headMatch = filters.head.length === 0 || filters.head.includes(noun.traits.head.seed.toString());
      const glassesMatch =
        filters.glasses.length === 0 || filters.glasses.includes(noun.traits.glasses.seed.toString());
      const bodyMatch = filters.body.length === 0 || filters.body.includes(noun.traits.body.seed.toString());
      const accessoryMatch =
        filters.accessory.length === 0 || filters.accessory.includes(noun.traits.accessory.seed.toString());
      const treasuryNounMatch =
        !filters.heldByTreasury || isAddressEqual(noun.owner, CHAIN_CONFIG.addresses.nounsTreasury);
      const instantSwapMatch =
        !filters.heldByNounsErc20 || isAddressEqual(noun.owner, CHAIN_CONFIG.addresses.nounsErc20);
