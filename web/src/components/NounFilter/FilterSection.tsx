...
export function FilterSection({ traitType, traits }: FilterSectionProps) {
  ...
  const filteredTraits = useMemo(() => {
    return traits.filter((trait) => trait.name.toLowerCase().includes(searchFilter.toLowerCase()));
  }, [traits, searchFilter]);

  return (
    <AccordionItem value={traitType}>
      <AccordionTrigger>{capitalizeFirstLetterOfEveryWord(traitType)}</AccordionTrigger>
        ...
        <div className="flex max-h-[500px] flex-col gap-2 overflow-y-auto">
          {filteredTraits.length > 0 ? (
            filteredTraits.map((trait) => (
              <FilterTraitItem traitType={traitType} trait={trait} key={traitType + trait.seed} />
            ))
          ) : (
            <div className="text-content-secondary flex w-full items-center justify-center">No results.</div>
          )}
        </div>
      </AccordionContent>
    </AccordionItem>
