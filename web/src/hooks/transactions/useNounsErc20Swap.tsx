...
import { nounsErc20TokenConfig, nounsNftTokenConfig } from "@/data/generated/wagmi";
  ...
  const swapValidation = useCallback(
    async (fromNounId: bigint, toNounId: bigint): Promise<CustomTransactionValidationError | null> => {
      ...
      if (!address) {
        ...
        return new CustomTransactionValidationError("SENDER_NOT_OWNER", "Sender does not own the swap input Noun.");
        ...
        return new CustomTransactionValidationError(
          ...
          "The Nouns ERC-20 contract does not own the swap output Noun."
        );
